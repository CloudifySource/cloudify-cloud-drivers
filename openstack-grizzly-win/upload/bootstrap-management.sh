...
if [ "$GIGASPACES_AGENT_ENV_JAVA_URL" = "NO_INSTALL" ]; then
	...
	download "JDK" $GIGASPACES_AGENT_ENV_JAVA_URL $WORKING_HOME_DIRECTORY/java.zip 101
	chmod +x $WORKING_HOME_DIRECTORY/java.zip
	...
	echo Installing JDK
	$WORKING_HOME_DIRECTORY/java.zip < $WORKING_HOME_DIRECTORY/input.txt > /dev/null
	mv ~/java/*/* ~/java || error_exit $? 103 "Failed moving JDK installation"
	rm -f $WORKING_HOME_DIRECTORY/input.txt
    export JAVA_HOME=~/java
    rm -f $WORKING_HOME_DIRECTORY/java.zip || error_exit $? 136 "Failed deleting java.zip from home directory"
fi  
