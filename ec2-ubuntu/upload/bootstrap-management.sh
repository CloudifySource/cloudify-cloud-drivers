...
#   $CLOUD_FILE  - Path to cloud configurations file
#############################################################################
...
if [ ! -f "/usr/bin/unzip" ]; then
	...
	sudo cp $WORKING_HOME_DIRECTORY/unzip /usr/bin || error_exit $? "Failed copying unzip"
	sudo cp $WORKING_HOME_DIRECTORY/unzipsfx /usr/bin || error_exit $? "Failed copying unzip"
fi


if [ ! -z "$CLOUDIFY_LINK" ]; then
	echo Downloading cloudify installation
	wget -q $CLOUDIFY_LINK.zip -O $WORKING_HOME_DIRECTORY/gigaspaces.zip || error_exit $? "Failed downloading cloudify installation"
fi
